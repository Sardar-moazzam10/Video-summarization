...
const HistoryPage = () => {
  ...
  const handleClick = (item) => {
    if (item.type === 'search') {
      ...
      navigate('/video-player', { state: { videoId: item.videoId } });
    } else if (item.type === 'transcript-view') {
      navigate('/transcript-viewer', { state: { videoId: item.videoId } });
    }
  };
        ...
        <div className="form-section">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            >
              ...
              <option value="transcript-view">Transcript</option>
            </select>
          ...
          <ul style={{ marginTop: '20px', paddingLeft: '0' }}>
            {filteredHistory.map((item, index) => (
              <li key={index} style={{
                ...
                <div onClick={() => handleClick(item)} style={{ flex: 1 }}>
                  <strong>
                    {item.type === 'search' ? '🔍 Search:' :
                     item.type === 'watch' ? '🎬 Watched:' :
                     '📝 Transcript:'}
                  </strong> {item.query || item.title || item.videoId}
            ...
            {filteredHistory.length === 0 && (
              <p style={{ color: '#999' }}>No history found for selected type.</p>
            )}
          </ul>
        </div>
      </div>
